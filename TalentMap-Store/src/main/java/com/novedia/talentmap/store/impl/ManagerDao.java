...
public class ManagerDao extends SqlMapClientDaoSupport implements IDao<Manager> {
	...
	public Manager get(final Integer id) throws DataAccessException {
		return (Manager) this.getSqlMapClientTemplate().queryForObject(
				DBRequestsConstants.GET_MANAGER, id);
	}
	...
	public List<Colleague> getAllCollaborators(Integer managerId)
			throws DataAccessException {
		return (List<Colleague>) this.getSqlMapClientTemplate().queryForList(
				DBRequestsConstants.GET_ALL_COLLABORATOR_BY_MANAGER_ID,
				managerId);
	}
