 ...
 * 
 */
public class AdminAddSkillContent extends VerticalLayout implements
		ClickListener, ValueChangeListener, CloseListener {
	...
	private void buildFormSkillFields() {
		...
		Select categorySelect = new Select(
				ConstantsEnglish.ADMIN_CATEGORY_SELECT_NAME, containerCategory);
		categorySelect.setRequired(true);
		categorySelect.setImmediate(true);
		categorySelect.setItemCaptionMode(Select.ITEM_CAPTION_MODE_PROPERTY);
		categorySelect
				.setItemCaptionPropertyId(ConstantsEnglish.ADMIN_CAPTION_PROPERTY);
		categorySelect.addListener(this);

		// Select concept field
		BeanItemContainer<Concept> containerConcept = new BeanItemContainer<Concept>(
				Concept.class);
		for (Concept concept : adminService.getAllConcepts()) {
			containerConcept.addItem(concept);
		}

		Select conceptSelect = new Select(
				ConstantsEnglish.ADMIN_CONCEPT_SELECT_NAME, containerConcept);
		conceptSelect.setImmediate(true);
		conceptSelect.setItemCaptionMode(Select.ITEM_CAPTION_MODE_PROPERTY);
		conceptSelect
				.setItemCaptionPropertyId(ConstantsEnglish.ADMIN_CAPTION_PROPERTY);
		...
		// Tool field
		TextField toolField = new TextField(
				ConstantsEnglish.ADMIN_TOOL_SELECT_NAME);
		toolField
				.setNullRepresentation(ConstantsEnglish.ADMIN_NULL_REPRESENTATION);
		...
		this.formAddSkill.addField(ConstantsEnglish.ADMIN_CATEGORY_NAME,
				categorySelect);
		this.formAddSkill.addField(ConstantsEnglish.ADMIN_CONCEPT_NAME,
				conceptSelect);
	...
	public void buildButton() {
		...
		this.newCategory = new Button(
				ConstantsEnglish.ADMIN_NEW_CATEGORY_BUTTON);
	...
	public void refreshCategoriesAvailable() {
		...
		Select categorySelect = (Select) this.formAddSkill
				.getField(ConstantsEnglish.ADMIN_CATEGORY_NAME);
		BeanItemContainer<Category> containerCategory = new BeanItemContainer<Category>(
				Category.class);
		for (Category category : adminService.getAllCategories()) {
			containerCategory.addItem(category);
	...
	public void buttonClick(ClickEvent event) {
		...
		if (button.getCaption().equals(ConstantsEnglish.SAVE_CAPTION)) {
			try {
				...
				Concept conceptSelected = (Concept) this.formAddSkill.getField(
						ConstantsEnglish.ADMIN_CONCEPT_NAME).getValue();
				if (conceptSelected == null) {
					throw new InvalidValueException("Concept is null");
				}
				String toolName = (String) this.formAddSkill.getField(
						ConstantsEnglish.ADMIN_TOOL_NAME).getValue();
				Tool newTool = Tool.builder().name(toolName)
						.concept(conceptSelected).build();

				if (adminService.checkTool(newTool) == null) {
					// Appelle du service d'ajout de tool
					adminService.addTool(newTool);
					getWindow().showNotification(
							ConstantsEnglish.ADMIN_NEW_TOOL_CONFIRMATION,
							Notification.TYPE_HUMANIZED_MESSAGE);
				} else {
					getWindow().showNotification(
							ConstantsEnglish.ADMIN_NEW_TOOL_EXISTING,
				...
				this.formAddSkill
						.getField(ConstantsEnglish.ADMIN_CATEGORY_NAME)
						.setValue(null);
				this.formAddSkill.getField(ConstantsEnglish.ADMIN_CONCEPT_NAME)
						.setValue(null);
				this.formAddSkill.getField(ConstantsEnglish.ADMIN_TOOL_NAME)
						.setValue(null);
				this.formAddSkill.getField(ConstantsEnglish.ADMIN_TOOL_NAME)
						.setEnabled(false);
				this.formAddSkill.getField(ConstantsEnglish.ADMIN_CONCEPT_NAME)
				...
				getWindow().showNotification(
						ConstantsEnglish.ADMIN_NEW_TOOL_ERROR,
			...
			getWindow().addWindow(newCategoryWindow);
				...
				ConstantsEnglish.ADMIN_NEW_CONCEPT_BUTTON)) {
	...
	public void valueChange(ValueChangeEvent event) {
		...
		TextField toolField = (TextField) this.formAddSkill
				.getField(ConstantsEnglish.ADMIN_TOOL_NAME);
		Select conceptSelect = (Select) this.formAddSkill
				.getField(ConstantsEnglish.ADMIN_CONCEPT_NAME);
		toolField.setMaxLength(ConstantsEnglish.TOOL_NAME_MAX_LENGTH);

		if (event.getProperty().getValue() instanceof Category) {
			// on met à jour la liste des concepts disponibles en fonction de la
			// catégorie sélectionnée
			Category selectedCategory = (Category) event.getProperty()
					.getValue();
			List<Concept> conceptAvailable = adminService
					.getAllConceptByCategory(selectedCategory);
			...
			if (this.formAddSkill.getField(ConstantsEnglish.ADMIN_TOOL_NAME)
					.isEnabled()) {
				toolField.setEnabled(false);
			}
	...
	public void windowClose(CloseEvent e) {
		...
		this.formAddSkill.getField(ConstantsEnglish.ADMIN_CATEGORY_NAME)
				.setValue(null);
		this.formAddSkill.getField(ConstantsEnglish.ADMIN_TOOL_NAME)
				.setEnabled(false);
		this.formAddSkill.getField(ConstantsEnglish.ADMIN_CONCEPT_NAME)
