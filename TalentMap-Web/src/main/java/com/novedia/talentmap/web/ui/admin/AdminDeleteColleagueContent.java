...
public class AdminDeleteColleagueContent extends VerticalLayout implements
	...
	private static final String[] VisibleColumn = { "First Name", "Last Name",
			"Email", "Phone" };
	...
	private void fillColleagueContainer(String name) {
		List<Colleague> colleagueResult = colleagueService
				.getAllColleaguesByName(name);
	...
	public void buttonClick(ClickEvent event) {
		if (event.getButton().getCaption().equals(LABEL_BUTTON_DELETE)) { // delete
		...
		} else if (event.getButton().getCaption().equals(LABEL_BUTTON_CANCEL)) { // cancel
																					// button
			colleagueList.setValue(null);

		} else if (event.getButton().getCaption().equals(LABEL_BUTTON_NO)) { // no
																				// button
			getWindow().removeWindow(popup);
		} else if (event.getButton().getCaption().equals(LABEL_BUTTON_YES)) { // yes
																				// button
			getWindow().removeWindow(popup);
			...
			for (Colleague colleague : colleagueSelected) {
				mapNotification = adminService.deleteColleague(colleague
						.getId());
			}
	...
	public void valueChange(ValueChangeEvent event) {
		Set<Colleague> setItemSelected = (Set<Colleague>) colleagueList
				.getValue();
