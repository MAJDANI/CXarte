 ...
 * @created 21 mai 2012
 */
public class AddSkillPanel extends Panel implements ClickListener, ValueChangeListener, Validator, IObservable{
	...
	private static final String[] OPTIONS = new String[]{"Débutant", "Intermédiare", "Professionnel", "Maitrise", "Expert"};
	 ...
	 * @throws Exception
	 */
	public AddSkillPanel(Label categoryLabel, Label conceptLabel, Select toolSelect, Select frequencyUseSelect, Select noUsingTimeSelect, RatingStars stars, Map<Integer, String> valueOptions, Button validSkill, ISkillService skillService, ListSkill listSkill, Accordion skillTab) throws Exception {
	...
	private void buildSelect() throws Exception{
		...
		//Just fill the Tool Select, the Frequency Use Select and the No Using Time Select
		fillSelect();
	}
	...
	public void buttonClick(ClickEvent event) {
		...
		if(button == this.validSkill){
			...
			if(!isValid(toolSelect.getValue()) || !isValid(stars.getValue()) || 
				!isValid(frequencyUseSelect.getValue()) || !isValid(noUsingTimeSelect.getValue())){
				
				getWindow().showNotification("Les champs ne sont pas tous remplis", Notification.TYPE_ERROR_MESSAGE);
			}else{
	...
	private void addOneSkill(){
		...
		try {
			...
			Tool tool = skillService.getToolByName(this.toolSelect.getValue().toString());
			...
			for(FrequencyUse fu : FrequencyUse.values()){
				if(fu.getValue().equals(this.frequencyUseSelect.getValue().toString())){
					frequencyUseValue = fu.getId();
				}
			}
			
			for(TimeUse tu : TimeUse.values()){
				if(tu.getValue().equals(this.noUsingTimeSelect.getValue().toString())){
			...
			if(!hasSkill(tool.getId())){
				...
				getWindow().showNotification("Compétence ajoutée", Notification.TYPE_TRAY_NOTIFICATION);
				
				//TODO
				
				Map<Category, Map> mapSkill = this.skillService.getAllCollaboratorSkill(COLLAB_ID);
				...
				getWindow().showNotification("Vous possédez déjà cette compétence", Notification.TYPE_WARNING_MESSAGE);
	 ...
	 * @throws Exception
	 */
	private boolean hasSkill(String tId) throws Exception{
		
		int toolId = Integer.parseInt(tId);
		
		Skill skill = this.skillService.getSkillByToolId(COLLAB_ID, toolId);
	 ...
	 * @param conceptSelect the conceptSelect to set
	 */
	 ...
	 * @param toolSelect the toolSelect to set
	 */
	 ...
	 * @param frequencyUseSelect the frequencyUseSelect to set
	 */
	 ...
	 * @param noUsingTimeSelect the noUsingTimeSelect to set
	 */
	 ...
	 * @param skillTab the skillTab to set
	 */
