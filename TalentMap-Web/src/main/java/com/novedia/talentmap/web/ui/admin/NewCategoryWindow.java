...
public class NewCategoryWindow extends Window implements ClickListener {
	...
	private void buildForm() {
		...
		TextField categoryField = new TextField(
				ConstantsEnglish.NEW_CATEGORY_WINDOW_FIELD_LABEL);
		categoryField.setRequired(true);
		categoryField.setMaxLength(ConstantsEnglish.CATEGORY_NAME_MAX_LENGTH);

		this.newCategoryForm.addField(
				ConstantsEnglish.NEW_CATEGORY_WINDOW_CATEGORY, categoryField);
	}
	...
	public void buttonClick(ClickEvent event) {
		try {
			...
			String newCategoryName = (String) this.newCategoryForm.getField(
					ConstantsEnglish.NEW_CATEGORY_WINDOW_CATEGORY).getValue();
			Category newCategory = Category.builder().name(newCategoryName)
					.build();
			...
			if (adminService.checkCategory(newCategory) == null) {
				...
				CUtils.showMessage(
						ConstantsEnglish.ADMIN_NEW_CATEGORY_CONFIRMATION,
						getWindow());
				this.close();
			} else {
				getWindow().showNotification(
						ConstantsEnglish.ADMIN_NEW_CATEGORY_EXISTING,
						Notification.TYPE_WARNING_MESSAGE);
			}
		} catch (InvalidValueException e) {
			getWindow().showNotification(
					ConstantsEnglish.NEW_CATEGORY_WINDOW_ERROR,
