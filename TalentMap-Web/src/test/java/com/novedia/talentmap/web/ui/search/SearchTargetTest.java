...
public class SearchTargetTest {
	...
	/**
	 * For services concerning Skills, SearchTarget calls an ISkillService. This object is mocked here. 
	 */
	@Mock
	...
	@Test
	public void getListColleagueForTooIdCheckedReturnsAListEmpty() throws Exception {
		//--------------
		//Given
		//--------------
		//Données
		...
		Collection<Object> lesItemId =(Collection<Object>)treeSkills.getContainerDataSource().getItemIds();
		...
		// When
		//--------------
		Mockito.when(skillServiceMock.getAllColleagueIdByListToolId(listToolIdForCategory)).thenReturn(expectedColleagueList);

		List<Integer> listColleagueIdResult = new ArrayList<Integer>();
		Boolean atLeastOneIsSelected = searchTarget.parseList(listColleagueIdResult);
		//--------------
		// Then
		//--------------
	...
	private List<Integer> buildListToolIdForConcept1() {
		...
		listToolIdForConcept.add(24);		listToolIdForConcept.add(25);
		listToolIdForConcept.add(26);		listToolIdForConcept.add(27);
		listToolIdForConcept.add(28);		listToolIdForConcept.add(29);
	...
	private List<Integer> buildExpectedColleagueList1() {
		List<Integer> expectedColleagueList = new ArrayList<Integer>();
		expectedColleagueList.add(15);		expectedColleagueList.add(16);
		expectedColleagueList.add(17);		expectedColleagueList.add(18);
		expectedColleagueList.add(19);		expectedColleagueList.add(20);
		expectedColleagueList.add(23);		expectedColleagueList.add(24);
		expectedColleagueList.add(27);		expectedColleagueList.add(28);
		expectedColleagueList.add(29);		expectedColleagueList.add(30);
		expectedColleagueList.add(35);		expectedColleagueList.add(36);
	...
	private List<VSkill> buildVSkills() {
		...
		list.add(v6);		list.add(v7);		list.add(v8);		list.add(v9);		list.add(v10);
		list.add(v11);		list.add(v12);		list.add(v13);		list.add(v14);
		return list;
	}
	
	@Ignore
	public void getListColleagueForTooIdCheckedReturnsAListNotEmpty() throws Exception {
		...
		//Données
		Integer conceptId = 6;
		//Les id des tool appartenant au concept "IOC" (idConcept = 6)(category PHP)
		List<Integer> listToolIdForConcept = buildListToolIdForConcept1();
		
		//Construire l'arbre des compétences et le setter à notre searchTarget qui va s'en servir
		List<VSkill> listExpected = buildVSkills();
		Mockito.when(skillServiceMock.getAllVSkillOrdered()).thenReturn(listExpected);
		Tree treeSkills = searchTarget.buildTreeSkills();
		...
		Boucle: for( Object item : lesItemId ) {
			if (item  instanceof Concept) {
				...
				if(conceptId == concept.getId()) {
					...
					//ces façons de récupérer l'item renvoient null => inefficaces
...
//					treeSkills.select(i3);
		...
		// When
		//--------------
		Mockito.when(skillServiceMock.getAllColleagueIdByListToolId(listToolIdForConcept)).thenReturn(expectedColleagueList);
		Boolean atLeastOneIsSelected = searchTarget.parseList(listColleagueIdResult);
